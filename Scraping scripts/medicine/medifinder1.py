...
for each_url in extra:
    ...
    cc = conn.cursor()
    cc.execute("insert into medi (name,salts,strength,color,shape,imprint) values (%s,%s,%s,%s,%s,%s) ",([var.encode('latin-1','replace')],[var0.encode('latin-1','replace')],[var2.encode('latin-1','replace')],[var3.encode('latin-1','replace')],[var4.encode('latin-1','replace')],[var6.encode('latin-1','replace')]))
    conn.commit()
